...
function NonoBananaPage() {
  // const location = useLocation() // Removed for Next.js
  // const navigate = useNavigate() // Removed for Next.js
  ...
  useEffect(() => {
    const loadUserSettings = async () => {
      ...
      try {
        ...
        console.log('📦 User settings response:', responseData)
...
//     if (!isMobile) return
// 
// //     // SwipeHandler removed for Next.js compatibility
...
//   }, [isMobile])
  ...
  const generateImage = async () => {
    ...
    const makeApiCall = async (retryCount = 0) => {
      ...
      try {
        ...
        if (!apiKey) {
          ...
          console.logError('Gemini', 'User API Key Missing', { hasUserKey: false, userId: user?.id })
          throw new Error(error)
        }
        
        console.debug('Gemini API initialized', { model, hasUserApiKey: true, userId: user?.id })
        ...
        console.logRequest('Gemini', 'generateContent', {
          model,
        ...
        if (!response.ok) {
          ...
          console.logError('Gemini', 'API Request Failed', {
          ...
          if ((response.status === 429 || response.status === 503) && retryCount < maxRetries) {
            ...
            console.info(`${statusMessage}. Retrying in ${waitTime}ms`)
        ...
        if (retryCount < maxRetries && (error.message.includes('429') || error.message.includes('rate') || error.message.includes('503') || error.message.includes('überlastet'))) {
          ...
          console.info(`API error occurred, retrying in ${waitTime}ms`)
          await new Promise(resolve => setTimeout(resolve, waitTime))
          return makeApiCall(retryCount + 1)
        }
        throw error
    ...
    try {
      ...
      console.logResponse('Gemini', true, {
      ...
      if (data.candidates && data.candidates[0]) {
        console.debug('Candidate found in response')
        ...
        if (data.candidates[0].content && data.candidates[0].content.parts) {
          ...
          console.debug('Response parts found', { partsCount: parts.length })
          
          let resultText = ''
          let resultImage = null

          parts.forEach((part, index) => {
            console.debug(`Processing part ${index}`, { hasText: !!part.text, hasInlineData: !!part.inline_data })
            ...
            if (part.text) {
              ...
              console.debug(`Image found in part ${index}`)
              ...
              console.debug(`Image found in part ${index} (alternative format)`)
            }
          })

          console.debug('Processing complete', { hasText: !!resultText.trim(), hasImage: !!resultImage })

          // Extract token usage data from Gemini response
          console.debug('🔍 DEBUG: Full Gemini data structure:', data)
          const usageMetadata = data.usageMetadata || {}  // ← FIXED: usageMetadata not usage_metadata!
          console.debug('🔍 DEBUG: usageMetadata extracted:', usageMetadata)
          console.debug('Token usage extracted', {
          ...
          if (resultImage || resultText.trim()) {
            ...
            console.warn('No valid image or text found in response parts')
            throw new Error('Keine gültigen Daten in der Antwort erhalten')
          }
        } else {
          console.warn('No content.parts found in candidate')
          const endTime = Date.now()
          const duration = ((endTime - startTime) / 1000).toFixed(1)
          setGenerationTime(`${duration}s`)
      ...
      console.logError('Gemini', error, { operation: 'Image Generation' })
