...
export default function NanoBananaPage() {
    ...
    const loadUserSettings = async () => {
      ...
      try {
        ...
        if (data.settings) {
          ...
          // Reset face image states when new settings load
  ...
  // Face Image Loading Effect - Ensure image loads when userSettings changes
  useEffect(() => {
    if (userSettings?.main_face_image_url && showMainFaceImage) {
      console.log('🔄 Face image URL available, preloading:', userSettings.main_face_image_url)
      // Preload the image to ensure it's ready to display
      const img = new Image()
      img.onload = () => {
      ...
      img.onerror = () => {
        console.error('❌ Face image preload failed')
        setFaceImageError(true)
        setFaceImageLoaded(false)
      }
      img.src = decodeURIComponent(userSettings.main_face_image_url)
    }
  ...
  // V1 4x Generation Function - EXACT COPY
  const generate4Images = async () => {
    ...
    const makeSingleCall = async (index) => {
      try {
        ...
        // Build final prompt with personalization (EXAKT wie in handleGenerate)
        let finalPrompt = prompt
        if (showPersonalization && userSettings?.main_face_image_url && showMainFaceImage && userSettings) {
          const personalizationText = generatePersonalizationText()
          if (personalizationText) {
        ...
        // Hauptgesichtsbild hinzufügen (wie in handleGenerate)
        if (userSettings?.main_face_image_url && showPersonalization && showMainFaceImage) {
          try {
            const response = await fetch(userSettings.main_face_image_url)
            const blob = await response.blob()
        ...
        // Additional uploaded images (wie in handleGenerate)
        for (const image of images) {
          if (image.base64) {
            try {
              const base64Data = image.base64.split(',')[1]
        ...
        // Request Body (wie in handleGenerate)
        const requestBody = {
          contents: [{
            role: "user",
            parts: parts
        ...
        // API Call (wie in handleGenerate)
        const response = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`,
          {
            method: 'POST',
        ...
        // Extract image from response (wie in handleGenerate)
  ...
  const generate10Images = async () => {
    ...
    // Funktion für einzelne API-Call (EXAKTE KOPIE der echten generateImage Logik)
    const makeSingleCall = async (index: number) => {
      try {
        const apiKey = userSettings?.gemini_api_key
        const model = process.env.NEXT_PUBLIC_GEMINI_MODEL || 'gemini-2.5-flash-image'
    ...
    try {
      ...
      alert('Fehler bei 10x Generierung: ' + (error as Error).message)
    } finally {
          ...
          <div style={{
            ...
            >
              {userSettings?.main_face_image_url && userSettings.main_face_image_url.length > 0 && showMainFaceImage ? (
                <>
                  <img 
                    src={userSettings.main_face_image_url.startsWith('data:') ? userSettings.main_face_image_url : decodeURIComponent(userSettings.main_face_image_url)}
                    alt="Main Face"
                    onLoad={() => {
                      setFaceImageLoaded(true)
                      setFaceImageError(false)
                    ...
                    onError={(e) => {
                      setFaceImageError(true)
                      setFaceImageLoaded(false)
                      console.error('❌ Face image failed to load:', userSettings?.main_face_image_url)
                    }}
                    style={{
                      width: '100%',
                      height: '100%',
                      objectFit: 'cover',
                      opacity: faceImageError ? 0.5 : 1,
                ...
                >
                  {userSettings && userSettings.main_face_image_url && showMainFaceImage && !faceImageLoaded && !faceImageError ? (
        ...
        {userSettings?.main_face_image_url && showMainFaceImage && userSettings && (userSettings.hair_color || userSettings.eye_color || userSettings.skin_tone || userSettings.age_range) && (
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '10px', flexDirection: 'column', width: '100%' }}>
              ...
              >
                <span>🍌</span>
                <span>{loading ? 'Generiere...' : 'Bild generieren'}</span>
              </button>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button 
                  ...
                  disabled={!prompt.trim() || multiLoading || multiLoading10 || loading}
                  style={{ 
                    padding: '10px 20px',
                    ...
                    border: 'none',
                    borderRadius: '8px',
                    cursor: !prompt.trim() || multiLoading || multiLoading10 || loading ? 'not-allowed' : 'pointer',
                    fontWeight: '600',
                    fontSize: '0.95rem',
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  <span>🍌🍌🍌🍌</span>
                  <span>{multiLoading ? `4x Gen... ${multiTimer}s` : '4x Generierung'}</span>
                </button>
                ...
                >
                  <span>🍌🍌🍌🍌🍌🍌🍌🍌🍌🍌</span>
                  <span>{multiLoading10 ? `10x Gen... ${multiTimer10}s` : '10x Generierung'}</span>
                </button>
              </div>
            </div>
          </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                {multiResults[0]?.generationTime && (
                  <span style={{
                    ...
                    ⏱️ {multiResults[0].generationTime}s
                  </span>
                )}
              </div>
              <button
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                {multiResults10[0]?.generationTime && (
                  <span style={{
                    ...
                    ⏱️ {multiResults10[0].generationTime}s
                  </span>
                )}
              </div>
              <button
                onClick={() => {
                  multiResults10.forEach((result, index) => {
                    if (result.success && result.image) {
                      const link = document.createElement('a')
                      link.href = result.image
                      link.download = `nano-banana-10x-${index + 1}-${Date.now()}.png`
                      link.click()
                    }
                  })
                }}
