...
export default function NanoBananaPage() {
  ...
  // Development fallback user when not logged in
  const currentUser = user || { 
    username: 'emilia.berlin', 
    id: '5bcc1012-7b1b-4ac3-a2e6-3093d492d2c0', // Real UUID for emilia.berlin
    modelId: 'emilia-berlin'
  }
  
  // Development fallback settings
  const fallbackSettings = user ? null : {
    username: 'emilia.berlin',
    id: 'dev-user-1',
    modelId: 'emilia-berlin',
  ...
  const [userSettings, setUserSettings] = useState(fallbackSettings)
  ...
  useEffect(() => {
    const loadUserSettings = async () => {
      ...
      try {
        console.log('🔍 Loading user settings for:', currentUser.id)
        const response = await fetch(`http://localhost:3001/nano-banana/api/user/settings?userId=${currentUser.id}`)
